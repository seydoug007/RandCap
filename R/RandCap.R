...
#' randomization_object<-RandCapGen(
#'   n = 100,
#'   id_prefix = "ID_",
#'   block_prefix = "Block_",
#'   arms = c("A", "B"),
#'   block_sizes = c(2, 4),
#'   ratio = c(1, 1),# optionnal when equal allocation
...
RandCapSettings <- function(randomization_object, output_path = "Randomization_settings.pdf",
  ...
  # Variables
  r_version <- randomization_object$settings$R_version
  date_randomization <- randomization_object$settings$date
  seed <- if (is.null(randomization_object$settings$initial_seed)
  ) "No seed was specified" else randomization_object$settings$initial_seed
  treatment_arms <- paste(randomization_object$settings$treatment_arms, collapse = ", ")
  ratios <- if (is.null(randomization_object$settings$ratio)
  ) "Equal size ratio" else paste(randomization_object$settings$ratio,
                                  collapse = ", ")
  strat_vars <- randomization_object$settings$strat_vars
  strat_levels <- randomization_object$settings$strat_levels
  block_sizes <- paste(randomization_object$settings$block_sizes_input, collapse = ", ")
