...
This function assigns subjects to specified treatments within sequential blocks. Among the predefined block sizes, the function randomly determines the number of blocks for each size. Consequently, the total number of randomizations in the generated list may exceed n, as the allocation process depends on the random selection and distribution of block sizes.
...
The randomization object (rand_dev_list in this case) generated by the RandCapGen function does not directly provide the randomization list but instead contains all the information about the randomization, including the list itself. A detailed description of the randomization will be provided in a later chapter.
...
The randomization object is a central component of the package, storing all the essential information and outputs related to the randomization process. It includes:

*Randomization Tables*: Both the full and simplified versions of the randomization tables.

*Randomization Settings*: The configuration parameters used to generate the randomization.

*Balance Assessment Table*: A table summarizing the assessment of treatment balance across strata.

This object serves as the backbone for generating randomization lists and reports. Created by the RandCapGen function, it is also used as input for other key functions, including:

*RandCapBalance*: To assess and visualize the balance of randomization across strata.

*RandCapProd*: To finalize and generate production-ready randomization outputs.

*RandCapSettings*: To review or modify the randomization settings.

*RandCapTable*: To format and export randomization tables.

By consolidating all relevant data and settings into a single object, the package ensures consistency, simplifies downstream analysis, and enhances the reproducibility of the randomization process.
